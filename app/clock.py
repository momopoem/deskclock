...
def run_face_recognize_once() -> dict:
    ...
    Returns dict. Expected keys (best-effort): ok, found_face, is_hiroshi, confidence
    """
...
def main():
    ...
    state.light.hiroshi_until_mono = 0.0
            ...
            if state.light.enabled:
                if pir_v == 1:
                    ...
                    if pir_rise and (is_dark or disp_state in ("DIM", "OFF")):
                        ...
                        if isinstance(fr, dict) and fr.get("ok") and fr.get("is_hiroshi") is True:
                            # Personal grace window (monotonic)
                            state.light.hiroshi_until_mono = now_mono + HIROSHI_LIGHT_GRACE_SEC

                    # (Re)arm OFF deadline. If within Hiroshi grace, keep extending with the longer timeout.
                    if state.light.hiroshi_until_mono and now_mono < float(state.light.hiroshi_until_mono):
                    ...
                    if state.light.is_on and state.light.deadline_mono and (now_mono >= float(state.light.deadline_mono)):
                        ...
                        if (not state.light.hiroshi_until_mono) or (now_mono >= float(state.light.hiroshi_until_mono)):
