...
class TimeWidget:
    ...
    _nixie_cache: dict[_NixieGlyphKey, pygame.Surface] = {}
    _glyph_cache: dict[_GlyphCacheKey, pygame.Surface] = {}

    def _render_nixie_glyph(self, font: pygame.font.Font, text: str, color, *, glass: bool) -> pygame.Surface:
        key = _NixieGlyphKey(id(font), text, tuple(int(x) for x in color), bool(glass))
        cached = self._nixie_cache.get(key)
        if cached is not None:
            return cached

        warm = tuple(int(x) for x in color)
        core = _mix_color(warm, (255, 236, 205), 0.28)
        glow = _mix_color(warm, (120, 48, 6), 0.30)
        text_main = font.render(text, True, core)
        text_glow = font.render(text, True, glow)

        pad_x = max(6, text_main.get_width() // 12)
        pad_y = max(5, text_main.get_height() // 10)
        surf_w = text_main.get_width() + pad_x * 2
        surf_h = text_main.get_height() + pad_y * 2
        surf = pygame.Surface((surf_w, surf_h), pygame.SRCALPHA)

        if glass:
            glass_rect = pygame.Rect(1, 1, surf_w - 2, surf_h - 2)
            glass_fill = (48, 18, 2, 22)
            glass_line = (164, 95, 34, 78)
            glass_inner = (255, 194, 132, 18)
            pygame.draw.rect(surf, glass_fill, glass_rect, border_radius=max(8, surf_h // 3))
            pygame.draw.rect(surf, glass_line, glass_rect, width=1, border_radius=max(8, surf_h // 3))
            hi = pygame.Rect(glass_rect.left + 2, glass_rect.top + 2, max(4, glass_rect.width - 4), max(3, glass_rect.height // 3))
            pygame.draw.rect(surf, glass_inner, hi, border_radius=max(6, hi.height // 2))

        tx = pad_x
        ty = pad_y
        for off in ((-1, 0), (1, 0), (0, -1), (0, 1)):
            surf.blit(text_glow, (tx + off[0], ty + off[1]))
        surf.blit(text_main, (tx, ty))

        self._nixie_cache[key] = surf
        return surf

    def _render_cached_glyph(self, font: pygame.font.Font, text: str, color) -> pygame.Surface:
        key = _GlyphCacheKey(id(font), text, tuple(int(x) for x in color))
    ...
    def _calc_digit_cell_width(self, font: pygame.font.Font) -> int:
        return max(1, max(font.size(str(d))[0] for d in range(10)))

    def _blit_centered_digit(self, canvas: pygame.Surface, surf: pygame.Surface, cell_x: int, cell_w: int, y: int) -> None:
        x = cell_x + (cell_w - surf.get_width()) // 2
        canvas.blit(surf, (x, y))

    def _render_nixie_fixed(
        self,
        *,
        ...
        hm_digits = (hour_tens_char or ' ', hour_ones_char, colon_min[-2], colon_min[-1])
        sec_digits = (ss[0], ss[1])

        main_cell_w = self._calc_digit_cell_width(font_7seg_main)
        sec_cell_w = self._calc_digit_cell_width(font_7seg_sec)
        colon_main = self._render_cached_glyph(font_7seg_main, ':', color)
        colon_sec = self._render_cached_glyph(font_7seg_sec, ':', color)
        surf_ampm = self._render_cached_glyph(font_ampm, ampm, color) if ampm else make_blank_slot(AMPM_SLOT_W, font_ampm.get_height())

        main_h = font_7seg_main.get_height()
        sec_h = font_7seg_sec.get_height()

        main_group_w = (main_cell_w * 4) + (colon_main.get_width() * 1)
        sec_group_w = (sec_cell_w * 2)
        total_w = AMPM_SLOT_W + GAP_AMPM + main_group_w + sec_group_w

        x_top = (w - total_w) // 2
        y_top = (h - main_h) // 2 - int(font_main_size * 0.35)
        ampm_raise_px = max(2, main_h // 18)
        y_ampm = y_top + main_h - surf_ampm.get_height() - ampm_raise_px
        y_sec = y_top + main_h - sec_h

        canvas.blit(surf_ampm, (x_top, y_ampm))
        x_main = x_top + AMPM_SLOT_W + GAP_AMPM

        hm_surfs = [self._render_cached_glyph(font_7seg_main, d, color) for d in hm_digits]

        # HH
        self._blit_centered_digit(canvas, hm_surfs[0], x_main + main_cell_w * 0, main_cell_w, y_top)
        self._blit_centered_digit(canvas, hm_surfs[1], x_main + main_cell_w * 1, main_cell_w, y_top)
        colon1_x = x_main + main_cell_w * 2
        canvas.blit(colon_main, (colon1_x, y_top))
        mm_x = colon1_x + colon_main.get_width()
        self._blit_centered_digit(canvas, hm_surfs[2], mm_x + main_cell_w * 0, main_cell_w, y_top)
        self._blit_centered_digit(canvas, hm_surfs[3], mm_x + main_cell_w * 1, main_cell_w, y_top)

        sec_digits_x = x_main + main_group_w
        sec_surfs = [self._render_cached_glyph(font_7seg_sec, d, color) for d in sec_digits]
        self._blit_centered_digit(canvas, sec_surfs[0], sec_digits_x + sec_cell_w * 0, sec_cell_w, y_sec)
        self._blit_centered_digit(canvas, sec_surfs[1], sec_digits_x + sec_cell_w * 1, sec_cell_w, y_sec)
        ...
        if WEATHER_ICON_ENABLE:
            ...
            if kind != "unknown":
                base_h = max(8, int(sec_h * 0.92))
                icon_h = int(base_h * max(WEATHER_ICON_SCALE, 1.0))
                avail_h = max(6, (y_sec - y_top) - max(0, WEATHER_ICON_MARGIN_Y - 2))
                icon_h = min(icon_h, avail_h)
                icon_w = max(sec_cell_w * 2, int(icon_h * 1.18))
                icon_rect = pygame.Rect(sec_digits_x, y_top + WEATHER_ICON_RAISE_PX, icon_w, icon_h)
                if lcd_mode:
        ...
        time_hour_w = AMPM_SLOT_W + GAP_AMPM + (main_cell_w * 2)
        time_rect_canvas = pygame.Rect(x_top, y_top, max(1, time_hour_w), main_h)
        return TimeWidgetResult(top_width=int(total_w), time_rect_canvas=time_rect_canvas, weather_rect_canvas=weather_rect_canvas)

    def _render_nixie_time(
        self,
        *,
        canvas: pygame.Surface,
        w: int,
        ...
        main_text = f"{hour_tens_char or ' '}{hour_ones_char}{colon_min}"
        surf_ampm = font_ampm.render(ampm, True, color) if ampm else None
        surf_main = self._render_nixie_glyph(font_7seg_main, main_text, color, glass=bool(getattr(theme_spec, 'nixie_glass', True)))
        surf_sec = self._render_nixie_glyph(font_7seg_sec, ss, color, glass=bool(getattr(theme_spec, 'nixie_glass', True)))

        gap_sec = max(10, font_7seg_main.get_height() // 14)
        total_w = surf_main.get_width() + (surf_ampm.get_width() + GAP_AMPM if surf_ampm else 0)
        x_top = (w - total_w) // 2
        y_top = (h - surf_main.get_height()) // 2 - int(font_main_size * 0.38)

        x_cur = x_top
        if surf_ampm is not None:
            y_ampm = y_top + surf_main.get_height() - surf_ampm.get_height() - max(2, font_main_size // 14)
            canvas.blit(surf_ampm, (x_cur, y_ampm))
            x_cur += surf_ampm.get_width() + GAP_AMPM

        canvas.blit(surf_main, (x_cur, y_top))
        main_rect = pygame.Rect(x_cur, y_top, surf_main.get_width(), surf_main.get_height())

        sec_x = main_rect.right - surf_sec.get_width() + max(4, font_7seg_main.get_height() // 10)
        sec_y = main_rect.bottom + gap_sec
        canvas.blit(surf_sec, (sec_x, sec_y))
        ...
        if WEATHER_ICON_ENABLE:
            ...
            kind = weather_code_to_icon(wcode)
            if kind != "unknown":
                icon_h = max(8, int(surf_sec.get_height() * 0.58 * WEATHER_ICON_SCALE))
                icon_w = max(surf_sec.get_width(), icon_h)
                icon_x = sec_x
                icon_y = max(2, y_top - icon_h - WEATHER_ICON_MARGIN_Y + WEATHER_ICON_RAISE_PX)
        ...
        time_hour_w = (main_rect.left - x_top) + max(1, int(surf_main.get_width() * 0.72))
        time_rect_canvas = pygame.Rect(x_top, y_top, time_hour_w, surf_main.get_height())
        top_width = max(total_w, (sec_x + surf_sec.get_width()) - x_top)
    ...
    def render(
        ...
        *,
        ...
        if theme_name == "nixie":
            return self._render_nixie_fixed(
                canvas=canvas,
                w=w,
                h=h,
                color=color,
